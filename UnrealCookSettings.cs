...
{
	public class StringParameterAttribute : Attribute
	{
		public string Enabled;

		public StringParameterAttribute(string Enabled)
		{
			this.Enabled = Enabled;
		}

		public string GetValue(string v)
			=> !v.IsNullOrWhiteSpace()
				? $"{Enabled}=\"{v}\""
				: string.Empty;
	}

	public class StringListParameterAttribute : Attribute
	{
		public string Enabled;

		public StringListParameterAttribute(string Enabled)
		{
			this.Enabled = Enabled;
		}
	...
	public class BoolParameterAttribute : Attribute
	{
		...
		public BoolParameterAttribute(string Enabled, string Disabled = "", string Null = "")
		{
			this.Enabled = Enabled;
	...
	//* -stage
	//* -package
	//* -stagingdirectory=D:/Pixelynx/Globant-Dev/PixelynxClient/Packages/
	// -cmdline=" -Messaging" -addcmdline="-SessionId=7980A2934A13F16FB7D3999D084A82CD -SessionOwner='igor.karatayev' -SessionName='Patch Game' "
	...
	public class UnrealCookSettings
	{
		...
		public override string ToString()
			=> GetType().EnumFieldsWithAttribute<StringParameterAttribute>()
					.Select(x => x.Attribute.GetValue(x.Field.GetValue<string>(this)))
					.Where(x => !x.IsNullOrWhiteSpace())
					.Select(x => $"-{x}")
				.Concat(GetType().EnumFieldsWithAttribute<BoolParameterAttribute>()
					.Select(x => x.Attribute.GetValue(x.Field.GetValue<bool?>(this)))
					.Where(x => !x.IsNullOrWhiteSpace())
					.Select(x => $"-{x}"))
				.Join(" ");
	}
